...
function validateRequest(value: unknown): ReferenceRequest {
  ...
  return { correlation_id: payload.correlation_id }
}

Deno.serve(async (request: Request) => {
  let parsedPayload: ReferenceRequest | null = null
  ...
  try {
    ...
    parsedPayload = validateRequest(decoded)
    ...
    const [unitsResponse, categoriesResponse, tagsResponse] = await Promise.all([
      serviceClient
        .from("measurement_units")
        .select("symbol,name,category,sort_order")
        .eq("is_active", true)
    ...
    if (unitsResponse.error || categoriesResponse.error || tagsResponse.error) {
      throw new HttpError(500, "reference_data_query_failed", "Could not load recipe reference data")
    }
    ...
    return jsonResponse(200, {
      ...
      result: {
        ...
        measurement_units: unitsResponse.data || [],
        recipe_categories: categoriesResponse.data || [],
        recipe_tags: tagsResponse.data || [],
      },
    })
  } catch (error) {
    ...
    console.error("recipe-import-reference-data failed", {
      correlation_id: parsedPayload?.correlation_id || null,
