...
export function AdminRecipeAutomationPage() {
    ...
    setForm((current) => ({
      ...
      targets:
        quantity <= 0
          ...
          : [
              ...current.targets.filter((target) => target.category_slug !== slug),
              { category_slug: slug, quantity },
            ],
    }))
  }

  const toggleCategory = (slug: string) => {
    updateTarget(slug, (targetMap.get(slug) || 0) > 0 ? 0 : 1)
  }
  ...
  const canSave =
    form.schedule.days_of_week.length > 0 &&
    totalRecipes <= maxRecipes &&
    (!form.enabled || totalRecipes > 0)

  if (isLoading || !initialized) {
    return <LoadingState message="Carregando automação de receitas..." />
  }
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <PageHeader
        ...
        subtitle="Defina a geração automática e acompanhe o histórico de receitas criadas pela automação."
      />
        ...
        <TabsList variant="pill" className="max-w-full overflow-x-auto">
          ...
          <TabsTrigger value="historico">
            ...
            {generatedRecipes.length > 0 && (
              <span className="ml-1 rounded-full bg-current/10 px-1.5 py-0.5 text-[10px] font-black">
        ...
        <TabsContent value="configuracao" className="space-y-8">
          <section className="grid grid-cols-1 gap-4 md:grid-cols-3">
            <div className="rounded-2xl border bg-white p-5 shadow-sm">
              <div className="flex items-center justify-between gap-3">
                ...
                <button
                  ...
                  className={cn(
                    ...
                    form.enabled ? 'bg-[#f76f25]' : 'bg-slate-200',
                  )}
                >
                  <span
                    className={cn(
              ...
              <p className="mt-3 text-xs text-slate-500">
                O workflow n8n consulta essa configuração a cada 5 minutos.
              </p>
            </div>
              ...
              <div className="mt-2 flex items-center gap-2">
                <span
                  className={cn(
                    ...
                    activeRun ? 'bg-emerald-500' : pendingManual ? 'bg-amber-500' : 'bg-slate-300',
                  )}
                />
                <p className="text-lg font-black text-slate-800">
                  {activeRun ? 'Executando' : pendingManual ? 'Execução solicitada' : 'Aguardando'}
                </p>
              </div>
              <p className="mt-3 text-xs text-slate-500">
                {activeRun
                  ? `Iniciada em ${formatDateTime(data?.runtime?.active_run_started_at)}`
                  : pendingManual
                    ? 'A solicitação manual será consumida no próximo ciclo.'
            ...
            <div className="rounded-2xl border bg-white p-5 shadow-sm">
              ...
              <p className="mt-3 text-xs text-slate-500">
                {lastRun
                  ? `${lastTotals?.created || 0} criadas · ${lastTotals?.failed || 0} falhas · ${formatDateTime(lastRun.completed_at)}`
            ...
            <div className="flex flex-col gap-2 border-b px-6 py-5 md:flex-row md:items-center md:justify-between">
              <div>
                <div className="flex items-center gap-2">
                  <Bot className="h-5 w-5 text-[#f76f25]" />
                  <h2 className="font-black text-slate-800">Receitas por categoria</h2>
                </div>
                <p className="mt-1 text-xs text-slate-500">
                  Selecione as categorias e a quantidade desejada em cada execução.
                </p>
              </div>
              <div
                className={cn(
                  'rounded-full px-4 py-2 text-xs font-black',
                  totalRecipes > maxRecipes ? 'bg-red-50 text-red-600' : 'bg-slate-100 text-slate-600',
                )}
              >
                Total: {totalRecipes} / {maxRecipes}
              </div>
            </div>
                  ...
                  >
                    ...
                    >
                      <span
                        className={cn(
                          ...
                          selected
                            ? 'border-[#f76f25] bg-[#f76f25] text-white'
                    ...
                    <div className="flex items-center gap-2 self-end sm:self-auto">
                      <Button
                        ...
                        size="icon"
                        disabled={!selected}
                        onClick={() => updateTarget(category.slug, Math.max(0, quantity - 1))}
                      ...
                      <Button
                        ...
                        variant="outline"
                        size="icon"
                        disabled={totalRecipes >= maxRecipes}
                        onClick={() => updateTarget(category.slug, quantity + 1)}
                        className="h-9 w-9 rounded-full"
                      >
          ...
          <section className="space-y-6 rounded-2xl border bg-white p-6 shadow-sm">
            <div className="flex items-start gap-3">
              <div className="rounded-xl bg-[#f76f25]/10 p-2 text-[#f76f25]">
                <CalendarDays className="h-5 w-5" />
              </div>
              <div>
                <h2 className="font-black text-slate-800">Programação</h2>
                <p className="text-xs text-slate-500">O horário é interpretado sempre em America/Sao_Paulo.</p>
              </div>
            </div>
            ...
            <div className="max-w-xs">
              ...
              <input
                ...
                className="h-11 w-full rounded-xl border border-slate-200 bg-white px-4 text-sm font-semibold text-slate-700 outline-none focus:border-[#f76f25]"
              />
            </div>
          </section>
          ...
          <section className="rounded-2xl border bg-white p-6 shadow-sm">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="font-black text-slate-800">Controle da automação</h2>
                </div>
                <p className="mt-1 text-xs text-slate-500">
                  Salvar altera as próximas execuções. “Executar agora” entra na fila e é reivindicado pelo n8n no próximo ciclo de 5 minutos.
                </p>
              </div>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <Button
                  ...
                  disabled={runNowMutation.isPending || totalRecipes === 0 || pendingManual}
                  className="rounded-full"
                >
            ...
            {!canSave && (
              <p className="mt-4 text-xs font-semibold text-red-600">
                Revise a configuração: escolha ao menos um dia, respeite o limite de {maxRecipes} receitas e,
                se a programação estiver ativa, selecione pelo menos uma categoria.
              </p>
            )}
          </section>
        </TabsContent>
          ...
          <section className="overflow-hidden rounded-2xl border bg-white shadow-sm">
            <div className="flex flex-col gap-3 border-b px-6 py-5 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="font-black text-slate-800">Histórico de receitas geradas</h2>
                </div>
                <p className="mt-1 text-xs text-slate-500">
                  Receitas criadas pela automação, da mais recente para a mais antiga.
                </p>
              </div>
              <div className="flex items-center gap-2">
                <span className="rounded-full bg-slate-100 px-4 py-2 text-xs font-black text-slate-600">
                  {generatedRecipes.length} receita{generatedRecipes.length === 1 ? '' : 's'}
                </span>
                <Button
                  type="button"
                  variant="outline"
            ...
            {generatedRecipes.length === 0 ? (
              <div className="px-6 py-14 text-center">
                ...
                <p className="mt-4 text-sm font-bold text-slate-600">Nenhuma receita gerada pela automação.</p>
                <p className="mt-1 text-xs text-slate-400">
                  As receitas criadas pelas próximas execuções aparecerão aqui.
                </p>
              </div>
            ) : (
                ...
                <table className="w-full min-w-[680px] text-left">
                  <thead className="bg-slate-50/80">
                    <tr className="text-[10px] font-black uppercase tracking-wider text-slate-400">
                      ...
                      <th className="px-6 py-3 text-right">Edição</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100">
                    {generatedRecipes.map((recipe) => (
                      <tr key={recipe.id} className="transition-colors hover:bg-slate-50/60">
                        ...
                        <td className="px-6 py-4 text-right">
                          <Link
                            to={`/admin/receitas/${recipe.id}`}
                            className="inline-flex items-center gap-1.5 rounded-full border border-slate-200 bg-white px-3 py-2 text-xs font-bold text-slate-600 no-underline transition-colors hover:border-[#f76f25]/30 hover:bg-[#f76f25]/5 hover:text-[#f76f25]"
                          >
                            Editar receita
